...
import pl.meleride.commands.context.ContextParser;
...
public abstract class Commands {
  ...
  public void handleCommand(CommandSender sender, Command command, String label, String[] args) {
    this.handleCommand(sender, command, label, args, new ContextParserImpl());
  }
  ...
  public List<String> handleCompleter(CommandSender sender, Command command, String label,
      String[] args) {
    return this.handleCompleter(sender, command, label, args, new ContextParserImpl());
  }
  ...
  public void registerCommandMethod(Method method, Object object, CommandInfo info) {
    ...
    if (!info.completer().isEmpty()) {
      try {
        completer = object.getClass().getDeclaredMethod(info.completer(), CommandSender.class,
